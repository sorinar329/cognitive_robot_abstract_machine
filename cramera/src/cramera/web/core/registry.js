...
(function () {
  ...
  function mountInto(slotEl, id) {
    ...
    try {
      ...
      const message = id === 'robot-scene' && /WebGL/i.test(String(err))
        ? 'The 3D scene needs WebGL, but WebGL is unavailable in this browser. ' +
          'Open this address in a browser with WebGL enabled. EQL and graph inspection remain available.'
        : 'panel "' + id + '" failed to mount — see console';
      root.innerHTML = '<div class="panel-error">' + message + '</div>';
    }
  }
