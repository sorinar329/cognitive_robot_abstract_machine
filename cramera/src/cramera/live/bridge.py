...
class Bridge:
    ...
    def bind(self) -> None:
        ...
        try:
            ...
            world_bodies_by_name: Dict[str, Body] = {}
            for body in world.bodies:
                basename = str(body.name).split("/")[-1]
                world_bodies_by_name[basename] = body
            ...
            # A loose object stays published once discovered, even on a bind whose
            # own re-scan misses it -- a demo that never re-parents an object once
            # picked up (see e.g. PickUpAction/PlaceAction's own disabled
            # AttachNode/DetachNode) keeps every loose object free-floating for
            # the rest of the run, so losing one here would otherwise hide it in
            # the viewer until a full page reload rebuilds the catalog from
            # scratch. Only kept while the body itself still exists in this
            # world, so a body actually removed between demo runs still drops.
            for key, body in self._bodies.items():
                if key != ROBOT_BASE_KEY and key not in bodies:
                    known_body = world_bodies_by_name.get(key)
                    if known_body is not None:
                        bodies[key] = known_body
