...
from typing_extensions import Callable, List, Optional, TYPE_CHECKING
...
class RosMarkerListener:
    ...
    _node: Optional[Node] = field(init=False, default=None)
    """
    The listener's own ROS node, while started.
    """

    _subscriptions: dict[str, Subscription] = field(init=False, default_factory=dict)
    """
    The live subscriptions by topic, so the viewer can add and remove them.
    """

    _executor: Optional[SingleThreadedExecutor] = field(init=False, default=None)
    """
    ...
    """
    The listener thread, joined when the session stops.
    """
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._spin, args=(self._executor,), daemon=True
        )
        self._thread.start()
    ...
    def _spin(self, executor: SingleThreadedExecutor) -> None:
        """
        Listen until this session or the surrounding ROS context stops.

        :param executor: The executor owned by this listener.
        """
        try:
            executor.spin()
    ...
    def _on_markers(self, topic: str) -> Callable[[MarkerArray], None]:
        """
        ...
        def deliver(array: MarkerArray) -> None:
            """
            Publish one received marker array.

            :param array: The received markers.
            """
