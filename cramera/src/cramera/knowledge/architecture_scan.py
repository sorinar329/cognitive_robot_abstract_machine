...
class ArchitectureScanner:
    ...
    ARCHITECTURE_CACHE_VERSION: ClassVar[int] = 4
    """
    Bumped whenever the cached scan's shape changes, so old caches are discarded.
    """

    GENERATED_MAPPING_FILE: ClassVar[str] = "ormatic_interface.py"
    """Generated database mappings omitted from the architecture graph."""
        ...
        for directory_path, directory_names, filenames in os.walk(base):
            ...
            for filename in filenames:
                if (
                    ...
                    or filename == self.GENERATED_MAPPING_FILE
                ):
                    continue
