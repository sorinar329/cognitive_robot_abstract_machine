...
if TYPE_CHECKING:
    from segmind.monitor import SegmindMonitor
...
class LiveEventDashboard:
    ...
    @classmethod
    def watching(
        cls, monitor: SegmindMonitor, address: Optional[DashboardAddress] = None
    ) -> Self:
        """
        A dashboard showing what ``monitor`` detects from now on and before.

        :param monitor: The monitor whose events are shown.
        ...
        monitor.listeners.append(dashboard.feed)
        return dashboard
