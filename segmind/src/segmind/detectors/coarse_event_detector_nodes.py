...
class AbstractInteractionDetector(AbstractDetector):
    ...
    def _find_interaction_events(
        ...
        :param primary_event_type: The event type to use as the primary signal (e.g.
            ``StopTranslationEvent`` for placing, ``TranslationEvent`` for pickup).
        ...
        for secondary in secondary_events:
            for primary in by_object.get(secondary.tracked_object, []):
                ...
                key = (
                    ...
                    secondary.tracked_object.id,
                    secondary.with_object.id,
                )
...
class PlacingDetector(AbstractInteractionDetector):
    """
    Represents a class detection mechanism for identifying and managing new placing
    events from observed system interactions.
    ...
    def update_context_and_events(
        ...
        actions logged in the system. It analyzes and filters specific event types to
        detect new events that can be generated. This function ensures distinct events
        are created by maintaining exclusivity through a pairing mechanism.
...
class PickUpDetector(AbstractInteractionDetector):
    ...
    def update_context_and_events(
        ...
        """
        Updates the context and generates a list of events based on translation events
        and loss of support events. The method identifies pairs of related events that
        are close in timestamp, determines if they are exclusive, and creates a new
        event when conditions are met.
