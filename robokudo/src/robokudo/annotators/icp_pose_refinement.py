    ...
    * Creates refined pose annotations
    ...
    def __init__(self, name="ICPPoseRefinementAnnotator", descriptor=Descriptor()):
        ...
        if ply_path is None or not ply_path.exists():
            self.logger.error("Couldn't lookup model path. Deactivating Annotator.")
    ...
    def get_model_path(self, class_name: str = ""):
        ...
        * Direct data path otherwise
        ...
        if self.descriptor.parameters.ros_pkg_path is not None:
            ...
            self.rk_logger.error(f"Can't look up necessary input data. Please check Parameters of this Annotator.'")
            return None
    ...
    def compute(self):
        ...
        * For each hypothesis with pose:
        ...
        if self.annotator_deactivated:
            error_message = f"{type(self).__name__} couldn't find model files and is deactivated."
        ...
        for annotation in annotations:
            ...
            # while iterating!
            pose_annotations = [x for x in object_hypothesis.annotations if
                                isinstance(x, robokudo.types.annotation.PoseAnnotation)]

            for pose_annotation in pose_annotations:
                # Transform PLY Model based on the PoseAnnotation for that object
                pose_t = robokudo.utils.transform.get_transform_matrix_from_q(numpy.asarray(pose_annotation.rotation),
                                                                              numpy.asarray(
                                                                                  pose_annotation.translation))
                ...
                upside_down_transform = \
                    robokudo.utils.transform.get_transform_matrix_for_rotation_around_axis(math.pi, (1, 0, 0))
                initial_pose_variants.append((pose_t @ upside_down_transform, "Upside down"))
                ...
                geometries_to_visualize.append({"name": "Initial Pose Estimate", "geometry": ply_model})
                ...
                for (pose_variant, description) in initial_pose_variants:
                    ...
                    pose_variant_registration_results.append((reg_p2p, pose_variant, description))
                    # self.rk_logger.info(reg_p2p)

                best_registration_result = None
                best_registration_result_idx = 0
                i = 0
                for (registration_result, pose_variant, description) in pose_variant_registration_results:
                    ...
                    if best_registration_result is None \
                            or registration_result.inlier_rmse * 1.1 < best_registration_result[0].inlier_rmse:
                ...
                self.rk_logger.info(f"Selected pose {best_registration_result_idx} as the best one")
                # Visualize ICP Result
                refined_ply_model = copy.deepcopy(self.class_name_to_ply_model['JeroenCup'])
                refined_ply_model.paint_uniform_color([0, 1, 0])
                # Transform PLY Model based on the Pose Annotation AND the ICP result
                refined_transformation = best_registration_result[0].transformation @ best_registration_result[1]
                refined_ply_model.transform(refined_transformation)

                geometries_to_visualize.append({"name": "Refined Pose Estimate", "geometry": refined_ply_model})

                refined_pose_annotation = robokudo.types.annotation.PoseAnnotation()
                refined_pose_annotation.translation = list(
                    robokudo.utils.transform.get_translation_from_transform_matrix(refined_transformation))
                refined_pose_annotation.rotation = list(
                    robokudo.utils.transform.get_quaternion_from_transform_matrix(refined_transformation))
