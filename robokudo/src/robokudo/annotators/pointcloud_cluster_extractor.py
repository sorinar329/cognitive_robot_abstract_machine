...
def generate_roi_with_mask_from_points(image_height: int, image_width: int,
                                       pc_cam_intrinsics: o3d.camera.PinholeCameraIntrinsic,
                                       cloud: o3d.geometry.PointCloud,
                                       color2depth_ratio: tuple[int,int] = (1, 1)) -> tuple[robokudo.types.cv.ImageROI, np.ndarray]:
    ...
    * Dilated mask to account for point sparsity
    ...
    :type pc_cam_intrinsics: o3d.camera.PinholeCameraIntrinsic
    :param cloud: Point cloud to project
    :type cloud: o3d.geometry.PointCloud
    :param color2depth_ratio: Scale ratio between color and depth images, defaults to (1, 1)
    :type color2depth_ratio: tuple, optional
    :return: Tuple of (ROI with mask, full image mask)
    :rtype: tuple(robokudo.types.cv.ImageROI, numpy.ndarray)
    """
    ...
    roi.mask = full_mask[roi.roi.pos.y: roi.roi.pos.y + roi.roi.height,
...
def cluster_points(input_cloud: o3d.geometry.PointCloud, eps: float, min_points: int,
                   rk_logger: logging.Logger) -> list:
    """Cluster points using DBSCAN algorithm.

    :param input_cloud: Point cloud to cluster
    :type input_cloud: o3d.geometry.PointCloud
    ...
    labels = np.array(
        input_cloud.cluster_dbscan(eps=eps, min_points=min_points, print_progress=print_progress))
...
class PointCloudClusterExtractor(robokudo.annotators.core.ThreadedAnnotator):
    ...
    class Descriptor(robokudo.annotators.core.BaseAnnotator.Descriptor):
        ...
        class Parameters:
            ...
            :ivar dbscan_min_cluster_count: Minimum points for DBSCAN cluster, defaults to 90
            :type dbscan_min_cluster_count: int
            :ivar min_cluster_count: Minimum total points for valid cluster, defaults to 1000
            :type min_cluster_count: int
            :ivar min_on_plane_point_count: Minimum points above plane, defaults to 90
            :type min_on_plane_point_count: int
            :ivar eps: DBSCAN epsilon parameter (density threshold), defaults to 0.04
            :type eps: float
            """

            def __init__(self):
                self.dbscan_min_cluster_count = 90
                self.min_cluster_count = 1000
                self.min_on_plane_point_count = 90
                self.eps = 0.04  # Density/"Distance" parameter that is used to find neighbouring points.

        parameters = Parameters()  # overwrite the parameters explicitly to enable auto-completion

    def __init__(self, name="PointCloudClusterExtractor", descriptor=Descriptor()):
        ...
        :type descriptor: PointCloudClusterExtractor.Descriptor, optional
        """
        super().__init__(name, descriptor)

    def points_and_indices_from_plane(self, plane_model: robokudo.types.annotation.Plane,
                                      cloud: o3d.geometry.PointCloud) -> tuple:
        ...
        :param plane_model:
        :type plane_model: robokudo.types.annotation.Plane
        :param cloud:
        :type cloud: o3d.geometry.PointCloud
        ...
        vec_plane_obb_normal = np.matmul(np.array([0, 0, 1]),
                                         plane_obb.R)  # transform normal vector with plane rotation
        ...
        above_plane_obb.extent = np.array([plane_obb.extent[0], plane_obb.extent[1], 0.8])

        on_plane_obb = robokudo.utils.o3d_helper.put_obb_on_target_obb(above_plane_obb, plane_obb)
        on_plane_cloud = outlier_cloud.crop(on_plane_obb)
        on_plane_cloud_indices = on_plane_obb.get_point_indices_within_bounding_box(outlier_cloud.points)
        ...
        if on_plane_point_count < self.descriptor.parameters.min_on_plane_point_count:
            self.feedback_message = f"above plane point cloud has not enough " \
        ...
        return plane_obb, on_plane_obb, on_plane_cloud, on_plane_cloud_indices, outlier_cloud
    ...
    def compute(self):
        ...
        * Visualizes clusters with unique colors
        ...
        plane_models = self.get_cas().filter_annotations_by_type(robokudo.types.annotation.Plane)
        ...
        plane_obb, on_plane_obb, on_plane_cloud, on_plane_cloud_indices, outlier_cloud = \
        ...
        all_cluster_indices = cluster_points(input_cloud=on_plane_cloud, eps=self.descriptor.parameters.eps,
        ...
        for cluster_indices in all_cluster_indices:
            if len(cluster_indices) < self.descriptor.parameters.min_cluster_count:
                self.rk_logger.debug(f"Skipping cluster {cluster_idx} with {len(cluster_indices)} points")
                continue
            ...
            cluster_indices_in_outlier_cloud = np.take(on_plane_cloud_indices, cluster_indices)
            cluster_cloud = outlier_cloud.select_by_index(list(cluster_indices_in_outlier_cloud))
            ...
            clusters[cluster_idx]['vis_cloud'].paint_uniform_color(
                cmap.colors[(cluster_idx % 20)])  # limit to 0-19 range due to map size
            all_vis_clouds.append(clusters[cluster_idx]['vis_cloud'])

            self.rk_logger.debug(f"Extracted cluster {cluster_idx} with {len(cluster_cloud.points)} points")
            ...
            (object_hypothesis.roi, full_mask_cluster) = \
                generate_roi_with_mask_from_points(image_height=height, image_width=width,
...
class NaivePointCloudClusterExtractor(robokudo.annotators.core.ThreadedAnnotator):
    ...
    def __init__(self, name="NaivePointCloudClusterExtractor", descriptor=Descriptor()):
        ...
        :type name: str, optional
        :param descriptor: Configuration descriptor, defaults to Descriptor()
        :type descriptor: NaivePointCloudClusterExtractor.Descriptor, optional
        """
        super().__init__(name, descriptor)
    ...
    def compute(self):
        ...
        * Visualizes clusters with unique colors
        ...
        # Cluster the points above the plane to get a list of point indices for each cluster
        all_cluster_indices = cluster_points(input_cloud=cloud, eps=self.descriptor.parameters.eps,
                                             min_points=self.descriptor.parameters.dbscan_min_cluster_count,
                                             rk_logger=self.rk_logger)
