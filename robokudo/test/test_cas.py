...
class TestCASObject(object):
    ...
    def test_annotations(self, function_setup):
        ...
        classification_list = cas.filter_by_type(robokudo.types.annotation.Classification, rs_obj.annotations)
        color_list = cas.filter_by_type(robokudo.types.annotation.SemanticColor, rs_obj.annotations)
    ...
    def test_ohs_with_criteria(self, function_setup):
        ...
        cl = cas.filter_by_type_and_criteria(robokudo.types.annotation.Classification, rs_obj.annotations,
                                             {'classname': ('==', 'Spoon')})
        assert (len(cl) == 1)

        # Attribute checking is case-sensitive!
        cl = cas.filter_by_type_and_criteria(robokudo.types.annotation.Classification, rs_obj2.annotations,
                                             {'classname': ('==', 'spoon')})
        assert (len(cl) == 0)
        ...
        cl = cas.filter_by_type_and_criteria(robokudo.types.annotation.SemanticColor, rs_obj.annotations,
                                             {'color': ('==', 'red')})
        assert (len(cl) == 1)
