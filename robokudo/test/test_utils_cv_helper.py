...
from robokudo.utils.cv_helper import adjust_roi, get_hsv_for_rgb_color, crop_image, get_hsv_for_bgr_color, \
    get_scaled_color_image_for_depth_image, get_scale_coordinates, draw_rectangle_around_center, clamp_bounding_rect, \
    sanity_checks_bounding_rects, adjust_image_roi, adjust_mask, crop_image_roi
...
class TestUtilsCVHelper(object):
    ...
    @pytest.mark.parametrize(["image_dim_xy", "crop_xy", "crop_wh", "expected_yx_shape"], [
    ...
    def test_crop_image(self, image_dim_xy: tuple[int, int], crop_xy: tuple[int, int], crop_wh: tuple[int, int],
    ...
    def test_crop_image_roi(self, image_dim_xy: tuple[int, int], crop_xy: tuple[int, int], crop_wh: tuple[int, int],
                            expected_yx_shape: tuple[int, int]):
        image = np.zeros((image_dim_xy[0], image_dim_xy[1], 3), np.uint8)
    ...
    def test_get_scaled_color_image_for_depth_image(self, scale_factor: tuple[float, float],
        ...
        assert scaled_image.shape[:2] == (height * scaley,
                                          width * scalex), f"Image should be scaled to {scalex}, {scaley}"

    def test_get_scaled_color_image_for_depth_image_no_depth_ratio(self, annotator_in_pipeline: BaseAnnotator):
        cas = annotator_in_pipeline.get_cas()
        cas.views[robokudo.cas.CASViews.COLOR2DEPTH_RATIO] = None
        image = np.zeros((640, 480, 3), dtype=np.uint8)
        assert pytest.raises(RuntimeError, get_scaled_color_image_for_depth_image, cas, image)
    ...
    def test_get_scale_coordinates(self, scale_factor):
        ...
        assert np.all(scaled_coordinates == (coordinates[0] * scalex, coordinates[
            1] * scaley)), f"Coordinates should be scaled to {scalex}, {scaley}"
    ...
    def test_get_hsv_for_rgb_color(self, rgb: tuple[int, int, int], hsv: tuple[int, int, int]):
        hsv_from_rgb = get_hsv_for_rgb_color(rgb)
        assert np.all(hsv == hsv_from_rgb)

    @pytest.mark.parametrize(["bgr", "hsv"], [
        # All cannels the same
        ((0, 0, 0), (0, 0, 0)),
        ((255, 255, 255), (0, 0, 255)),
        ((int(255 / 2), int(255 / 2), int(255 / 2)), (0, 0, int(255 / 2))),
    ...
    def test_get_hsv_for_bgr_color(self, bgr: tuple[int, int, int], hsv: tuple[int, int, int]):
        hsv_from_bgr = get_hsv_for_bgr_color(bgr)
        assert np.all(hsv == hsv_from_bgr)

    @pytest.mark.parametrize(["center", "size", "expected_points"], [
        ...
        ((513, 513), (128, 128), ((128 / 2) - 1) * ((128 / 2) - 1)),  # Added center row/col outside of image
        # Uneven width and height with center outside image boundaries
        ((513, 513), (127, 127), ((127 // 2) - 1) * ((127 // 2) - 1)),  # Center row/col outside of image
        # Even width and height with center outside image boundaries
        ((-1, -1), (128, 128), ((128 / 2) * (128 / 2))),  # Added center row/col outside of image
        # Uneven width and height with center outside image boundaries
        ((-1, -1), (127, 127), (127 // 2) * (127 // 2)),  # Center row/col outside of image
    ])
    def test_draw_rectangle_around_center(self, center: tuple[int, int], size: tuple[int, int], expected_points: int):
        x, y = center
        w, h = size

        image = np.zeros((512, 512, 3), dtype=np.uint8)
        modified_image = draw_rectangle_around_center(image, x, y, w, h, value=(255, 0, 0))

        assert np.sum(modified_image != 0) == expected_points, f"Expected {expected_points} pixels to be set"
        assert image.shape == modified_image.shape, "Image shape changed"

    @pytest.mark.parametrize(["points", "adjusted"], [
        # No adjustment needed
        ((128, 128, 128, 128), (128, 128, 128, 128)),
        # Adjust width
        ((128, 128, 512, 128), (128, 128, 512 - 128 - 1, 128)),
        # Adjust height
        ((128, 128, 128, 512), (128, 128, 128, 512 - 128 - 1)),
        # Adjust x and width
        ...
        # Adjust all
        ((-64, -64, 768, 768), (0, 0, 512 - 1, 512 - 1)),
    ])
    def test_clamp_bounding_rect(self, points: tuple[int, int, int, int], adjusted: tuple[int, int, int, int]):
        image_width = 512
        image_height = 512
    ...
    @pytest.mark.parametrize(["image_dim", "roi", "offset"], [
        # Roi no change
    ...
    def test_adjust_roi_within_boundaries(self, image_dim: tuple[int, int], roi: tuple[int, int, int, int],
                                          offset: int):
        w, h = image_dim
    ...
    def test_adjust_roi_boundary_conditions(self, image_dim: tuple[int, int], roi: tuple[int, int, int, int],
                                            offset: int):
        w, h = image_dim
        image = np.zeros((h, w, 3), dtype=np.uint8)
        ...
        assert 0 <= new_x < w, f"{roi} x should be expanded by {offset} within image width"
        assert 0 <= new_y < h, f"{roi} y should be expanded by {offset} within image width"
        assert 1 <= new_width <= w, f"{roi} width should be at least 1 and within image width"
        assert 1 <= new_height <= h, f"{roi} width should be at least 1 and within image width"
        assert 1 <= new_x + new_width <= w, f"{roi} width should be expanded by {offset} within image width"
        assert 1 <= new_y + new_height <= h, f"{roi} height should be expanded by {offset} within image width"
    ...
    def test_adjust_image_roi_within_boundaries(self, image_dim: tuple[int, int], roi: tuple[int, int, int, int],
                                                offset: int):
        w, h = image_dim
    ...
    def test_adjust_image_roi_boundary_conditions(self, image_dim: tuple[int, int], roi: tuple[int, int, int, int],
                                                  offset: int):
        w, h = image_dim
        offset = offset
        ...
        assert 0 <= image_roi.roi.pos.x <= w, f"{roi} x should be expanded by {offset} within image width"
        assert 0 <= image_roi.roi.pos.y <= h, f"{roi} y should be expanded by {offset} within image width"
        assert 1 <= image_roi.roi.width <= w, f"{roi} width should be at least 1 and within image width"
        assert 1 <= image_roi.roi.height <= h, f"{roi} width should be at least 1 and within image width"
        assert 1 <= image_roi.roi.pos.x + image_roi.roi.width <= w, f"{roi} width should be expanded by {offset} within image width"
        assert 1 <= image_roi.roi.pos.y + image_roi.roi.height <= h, f"{roi} height should be expanded by {offset} within image width"

    @pytest.mark.parametrize(["mask", "offset", "fill_value"], [
