...
class TestUtilsNonMaximaSuppression(object):
    ...
    @pytest.mark.parametrize(["confidence_threshold", "filtered_count"], [
        (0.5, 1),
        (0.3, 2)
    ])
    def test_non_max_suppression_confidence_threshold(self, confidence_threshold: float, filtered_count: int):
        predictions = [
            ((10, 10, 20, 20), 0.4, "person"),
            ((15, 15, 25, 25), 0.8, "person")
        ]
        filtered = non_max_suppression_(predictions, confidence_treshold=confidence_threshold)
        assert len(filtered) == filtered_count

    @pytest.mark.parametrize(["iou_threshold", "filtered_count"], [
        (0.1, 1),
        (0.4, 2),
    ])
    def test_non_max_suppression_iou_threshold(self, iou_threshold: float, filtered_count: int):
        predictions = [
            ((10, 10, 20, 20), 0.9, "person"),
            ((15, 15, 25, 25), 0.8, "person")
        ]
    ...
    def test_class_based_nms_confidence_threshold(self, confidence_threshold: float, filtered_count: int):
        predictions = [
            ((10, 10, 20, 20), 0.4, "person"),
            ((15, 15, 25, 25), 0.8, "person")
        ]
        filtered = class_based_nms(predictions, confidence_threshold=confidence_threshold)
        assert len(filtered) == filtered_count
    ...
    def test_class_based_nms_iou_threshold(self, iou_threshold: float, filtered_count: int):
        predictions = [
            ((10, 10, 20, 20), 0.9, "person"),
            ((15, 15, 25, 25), 0.8, "person")
        ]
