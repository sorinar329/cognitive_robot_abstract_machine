...
class QPSolverGurobi(QPSolver):
    ...
    def init(self, qp_data: QPData):
        ...
        self.x = self.qpProblem.addMVar(qp_data.quadratic_weights.shape[0],
                                        lb=qp_data.box_lower_constraints,
                                        ub=qp_data.box_upper_constraints)
        H = sp.diags(qp_data.quadratic_weights, 0)
        self.qpProblem.setMObjective(Q=H, c=qp_data.linear_weights, constant=0.0, xQ_L=self.x, xQ_R=self.x, sense=GRB.MINIMIZE)
        try:
            self.qpProblem.addMConstr(qp_data.eq_matrix, self.x, gurobipy.GRB.EQUAL, qp_data.eq_bounds)
        except (GurobiError, ValueError) as e:
            pass  # no eq constraints
        try:
            self.qpProblem.addMConstr(qp_data.neq_matrix, self.x, gurobipy.GRB.GREATER_EQUAL, qp_data.neq_lower_bounds)
            self.qpProblem.addMConstr(qp_data.neq_matrix, self.x, gurobipy.GRB.LESS_EQUAL, qp_data.neq_upper_bounds)
        except (GurobiError, ValueError) as e:
