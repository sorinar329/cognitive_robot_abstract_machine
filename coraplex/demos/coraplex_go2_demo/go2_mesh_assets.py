"""
Downloads the Go2 MJCF scene's referenced meshes from ``mujoco_menagerie`` on first
use.
"""
...
MENAGERIE_ASSET_URL = (
    ...
    "/{revision}/unitree_go2/assets/{filename}"
)
"""
Template for one Go2 mesh in the ``mujoco_menagerie`` repository.
"""
...
class Go2MeshAssets:
    """
    The Go2 meshes a scene needs, downloaded from ``mujoco_menagerie`` on first use.
    ...
    def download_if_missing(self) -> Path:
        ...
        logger.info(
            "Downloading %d Go2 meshes from mujoco_menagerie@%s into %s",
            len(missing),
            self.revision,
            directory,
        )
