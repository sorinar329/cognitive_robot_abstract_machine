"""
Downloads the Panda meshes ``stacking_scene.xml`` references, from
``mujoco_menagerie``, on first use.

Ported from :class:`experiments.montessori.franka_panda_equipment`'s own
``PandaMeshAssets`` (``montessori_merge``), which solves the exact same problem for
the Montessori demo's copy of this scene.
"""
