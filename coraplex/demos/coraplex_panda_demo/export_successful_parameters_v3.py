"""
Third-generation copy of ``export_successful_parameters_v2.py``, turning
``build_merged_dataset_v3.py``'s successful stacking attempts into CSV files a joint
probability tree can be learned from.

Structured exactly like ``export_successful_parameters_v2.py``, reading this
generation's own separate database (``coraplex_panda_demo_v3_successful``) instead of
the ``coraplex_panda_demo_v2_successful`` archive: every action's learning columns
include every cube's final z position (see :data:`CUBE_FINAL_Z_COLUMNS`) -- not just the
height the step's picked cube actually settled at -- alongside its sampled
velocity/friction parameters, which is what lets ``causal_diagnosis_v3.py`` use the
whole stack's state, the demo's actual goal, as the causal circuit's effect role instead
of ``step_index``.

Run it with the interpreter whose packages point at this checkout, for example::

    /home/sorin/.virtualenvs/cram2-env/bin/python export_successful_parameters_v3.py
    /home/sorin/.virtualenvs/cram2-env/bin/python export_successful_parameters_v3.py --per-step
"""
...
attempts) had finished -- the outcome ``causal_diagnosis_v3.py`` registers as the causal
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-directory",
        type=Path,
        default=Path(__file__).parent / "datasets_v3",
