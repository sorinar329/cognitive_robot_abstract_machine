...
Samples each cube's first attempt from the same parameter range ``demo3.py`` samples its
own attempts from (see :func:`sample_actions`). ``inference3b.py`` is otherwise this same
file, differing only in sampling that first attempt from a wider, more failure-prone
range instead.

Run it with the interpreter whose packages point at this checkout, for example::

    /home/sorin/.virtualenvs/cram2-env/bin/python inference3a.py
"""
...
rather than collecting a large training dataset, and a failed cube can trigger several
correction attempts before moving on, making every iteration slower.
"""
...
def apply_correction(
    ...
    :data:`~inference3a.MAX_CORRECTION_ATTEMPTS_PER_CUBE` one parameter at a time instead
...
def sample_actions(
    ...
    """
    Sample a fresh pickup/place pair from the same priors ``demo3.py`` samples its own
    attempts from, for stacking ``object_body`` centered above ``target_body``, one cube
    height higher.

    Left at :func:`~pickup_place_parameterization.sample_pickup_instance` and
    :func:`~pickup_place_parameterization.sample_place_instance`'s own default priors
    (:data:`~pickup_place_parameterization.PICKUP_PARAMETER_PRIORS` and
    :data:`~pickup_place_parameterization.PLACE_PARAMETER_PRIORS`) rather than a wider
    set: this demo's causal correction is meant to raise ``demo3.py``'s own ~50% success
    rate on that same range, not to recover from failures a wider range would have
