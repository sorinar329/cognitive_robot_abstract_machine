"""
Second-generation copy of ``build_merged_dataset.py``, collecting ``demo3.py``'s usable
attempts -- including each cube's final z position -- into one flat dataset.

Structured exactly like ``build_merged_dataset.py``: each source run lives in its own
...
Only one source run is listed in :data:`SOURCE_RUNS` for now, since ``demo3.py`` has not
yet been run more than once -- add further :class:`SourceRun` entries the same way
``build_merged_dataset.py`` does once later ``demo3.py`` runs are archived under their own
database names.

Run it with the interpreter whose packages point at this checkout, for example::

    /home/sorin/.virtualenvs/cram2-env/bin/python build_merged_dataset_v2.py
"""
...
Extends ``build_merged_dataset.py``'s own :data:`DATASET_COLUMNS` with the five z
position columns ``demo3.py`` records on every attempt (see
``stacking_attempt_record_v2.py``).
"""
...
class SourceRun:
    ...
    generous placeholder is used here until ``demo3.py`` has actually been run and its
    own report inspected the same way.
    """
...
SOURCE_RUNS = [
    SourceRun(
        "coraplex_panda_demo_v2", 1_000_000, DEMO_DIRECTORY / "support_report_v2.md"
    ),
]
"""
The runs merged into the dataset, oldest first.

See :class:`SourceRun` on why ``healthy_iteration_limit`` is a placeholder for now.
"""
...
def read_iteration_outcomes(
    ...
    :return: Maps ``(run_started_at, iteration_index)`` to whether segmind approved the
        stack. Empty when there is no report. Keyed by ``(run_started_at,
        iteration_index)`` rather than bare iteration number: unlike
        ``build_merged_dataset.py``'s own archived sources, each of which held exactly
        ...
        ``run_started_at`` for exactly this reason -- see its own
    ...
    return {
        (datetime.datetime.fromisoformat(run_started_at), int(iteration)): approved
        == "True"
...
def collect_attempts(source: SourceRun) -> list[dict]:
    ...
    iteration and whether the iteration was cut short, which is what distinguishes a
    step that was never reached from one that ran.

    Grouped by ``(run_started_at, iteration_index)`` rather than bare iteration number
    -- see :func:`read_iteration_outcomes` for why bare iteration number is not enough
    to tell two different runs' attempts apart in ``demo3.py``'s database.
    """
...
def build_successful_subset() -> None:
    """
    Copy the attempts belonging to iterations that fully stacked into their own
    database.
...
def build_dataset() -> None:
    ...
    with engine.connect() as connection:
        ...
        total = connection.execute(
            text("SELECT count(*) FROM stacking_dataset")
        ).scalar()
        labelled = connection.execute(
            text(
                "SELECT count(*) FROM stacking_dataset WHERE iteration_stacked IS NOT NULL"
            )
        ).scalar()
