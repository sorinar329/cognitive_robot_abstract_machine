"""
Closed-loop Panda cube stacking: JPT-guided parameter sampling plus causal failure
diagnosis against the models ``training/train_jpt2.py`` fits.

A physically simulated Panda stacks three cubes into a tower, one attempt per cube. Each
attempt's :class:`~coraplex.robot_plans.actions.core.pick_up.PickUpAction` and
:class:`~coraplex.robot_plans.actions.core.placing.PlaceAction` are sampled from a wide
prior over their tunable velocity/timing fields (see
``pickup_place_parameterization.py``), deliberately making some fraction of attempts
fail. A failed cube is diagnosed against ``causal_diagnosis_v2``'s trees (fit on
successful attempts collected the same way, with every cube's own final z position as
the causal circuit's effect role, see that module's own docstring for why) and retried
with the diagnosed correction applied, up to
:data:`MAX_CORRECTION_ATTEMPTS_PER_CUBE` times.

The arm parks between the pickup and the place on every attempt, including every
corrective retry, rather than crossing directly from the pickup pose to the place pose:
once the stack has some height, the direct crossing can pass right through where the
already-stacked cubes are, and a corrective retry repeats that same crossing several
times in a row -- exactly where a collision with an already-stacked cube is most likely.

Run it with the interpreter whose packages point at this checkout, for example::

    /home/sorin/.virtualenvs/cram2-env/bin/python franka_stacking_inference_demo.py
"""
...
RANDOM_SEED = int(os.environ.get("STACKING_DEMO_RANDOM_SEED", "42"))
"""
Seed for the per-attempt parameter sampling.

Only makes the sampled parameters -- not the run as a whole -- reproducible: the
physics simulation and Giskard's own solve are not seeded, so the same seed can still
produce a different outcome between runs.
"""
...
# Without gravity compensation, the arm never registers as converged and Giskard
# keeps sending corrective commands indefinitely, stalling the rest of the plan.
...
NUMBER_OF_ITERATIONS = int(os.environ.get("STACKING_DEMO_NUMBER_OF_ITERATIONS", "20"))
"""
Number of times the full pickup/stack sequence is repeated.

Kept small: this demo illustrates the diagnose-and-correct loop rather than collecting
a large training dataset, and wide priors make every iteration slower (a failed cube
can trigger several correction attempts before moving on).
"""
...
Generous: a cube here can go through several correction attempts, each costing roughly
as long as a normal attempt.
"""
...
while the gripper is actually holding a cube. Slower than :class:`ParkArmsAction`'s own
default park speed, which is tuned for a park made with an empty gripper -- that same
speed jerks a cube, held by nothing but friction, right out of the fingers. Cut down
from 1.0 and then 0.3 rad/s in earlier tuning rather than settled on: tune it again if
it is still too fast (or needlessly slow).
"""
...
Generous compared to the scene's actual table/reach, so only a genuinely diverged
simulation (a cube launched off the table by a bad contact) trips it.
"""
...
PICKUP_MODEL_TO_LIVE_FIELD_NAMES: dict[str, str] = {
    "grasp_linear_velocity": "final_approach_linear_velocity",
    "grasp_stall_min_time": "grasp_stall_minimum_time",
}
"""
Maps a :data:`~causal_diagnosis_v2.PICKUP_CAUSAL_CONFIG` cause name -- fixed by
``training/train_jpt2.py``'s training data column names -- to :class:`PickUpAction`'s
own current field name, wherever the two differ.

The trained trees were fit against an older revision of :class:`PickUpAction` whose
fields this demo's own ``PickUpAction`` has since renamed (``grasp_linear_velocity`` ->
``final_approach_linear_velocity``, ``grasp_stall_min_time`` ->
``grasp_stall_minimum_time``); retraining against every rename is unnecessary as long as
every read of, or write to, a live action goes through this mapping instead.
:class:`~causal_diagnosis_v2.PlaceAction`'s own cause names already match its current
field names, so no equivalent mapping exists for it.
"""


def _live_field_name(model_field_name: str) -> str:
    """
    :return: ``model_field_name`` translated to the corresponding live action field
        name, or unchanged if the two already agree (see
        :data:`PICKUP_MODEL_TO_LIVE_FIELD_NAMES`).
    """
    return PICKUP_MODEL_TO_LIVE_FIELD_NAMES.get(model_field_name, model_field_name)


def _observed_parameters(action, field_names: tuple[str, ...]) -> dict[str, float]:
    """
    :return: ``action``'s current value for each of ``field_names`` (trained-tree
        column names, translated via :func:`_live_field_name` before reading), keyed by
        ``field_names`` themselves.
    """
    return {name: getattr(action, _live_field_name(name)) for name in field_names}
...
def apply_correction(
    ...
    :data:`MAX_CORRECTION_ATTEMPTS_PER_CUBE` one parameter at a time instead
    ...
    """
    corrected_values = {
        _live_field_name(correction.variable_name): correction.corrected_value
...
INFERENCE_REPORT_PATH = Path(__file__).parent / "franka_stacking_inference_report.md"
"""
...
def segmind_sees_on_floor(body: Body) -> bool:
    ...
    Checked against the floor rather than support from the cube below: a cube that
    missed its target but landed leaning against (rather than under) the cube it was
    meant to stack onto is not on the floor either, and should not be judged a failure
    by this check alone -- :func:`cube_is_stacked_on`'s own geometric check is what
...
def _apply_object_friction(pickup_action: PickUpAction) -> None:
    ...
    Addressed by geom name (``<body>_geom``, see ``stacking_scene.xml``) rather than
    body name: :meth:`~physics_simulators.mujoco_simulator.MujocoSimulator.set_geom_friction`
    sets one named geom's friction directly, unlike a body-addressed setter that would
    have to discover which geoms belong to it.
    """
    friction_result = multi_sim.simulator.set_geom_friction(
        f"{pickup_action.object_designator.name.name}_geom",
...
def _rotate_gripper_away_from(object_body: Body, target_body: Body) -> bool:
    ...
    A fixed ``rotate_gripper=True`` would keep the opening axis off the spawn row,
    which runs along y -- but only because every pickup would then start from that row.
    Here a cube can be retried after being knocked out of the row by an earlier failed
    attempt, landing close to the stack or the cube it is meant to stack onto
    (``target_body``, which for the first step is the bottom/support cube); a fixed roll
    then has even odds of sweeping the fingers straight into whichever cube is now
    nearby instead of away from it. Reading both cubes' current x/y position and
    rolling away from whichever direction ``target_body`` actually is keeps the same
    protection in the ordinary spawn-row case (the row runs along y, so this resolves to
    the same ``True`` a fixed choice would use there) while also covering the
    knocked-out-of-row case a fixed choice cannot.
...
def build_stack_plan(
    ...
    return sequential(
        [
            ...
            # stacked cubes are (see this module's own docstring). Slowed to
            # CARRYING_PARK_JOINT_VELOCITY: at full speed this move jerks the cube --
            # held by nothing but friction -- right out of the fingers.
...
def perform_attempt(
    ...
    failure instead of letting it propagate -- an attempt is judged by where the cube
    actually ended up (:func:`verify_stacked`), not by whether the plan raised, so a
    caught failure still gets a normal success/failure verdict instead of aborting the
    whole run.
...
class SimulationDidNotRecover(RuntimeError):
    """
    Raised when cubes are still outside the workspace after being reset.
    """
...
def print_iteration_summary(iteration_index: int) -> None:
    """
    Prints the final z-height of every cube.
    """
    heights = {
...
# Without this, the viewer falls back to MuJoCo's default camera instead of the
# scene's intended viewing angle.
...
with ExecutionEnvironment(
    execution_type=execution_mode,
    collision_avoidance=False,
    real_time_pacing=True,
    # Real-time pacing makes every tick roughly one control-loop period of wall-clock
    # time (see ExecutionEnvironment.real_time_pacing), so the default tick budget
    # (2000 per motion mapping) would let a single motion run for minutes before its
    # own timeout gives up on it. Cut down to whatever a normal motion actually needs
    # plus headroom, so a genuinely stuck motion fails fast instead of stalling the
    ...
    for iteration in range(1, NUMBER_OF_ITERATIONS + 1):
        ...
        for cube_to_pick, cube_to_stack_on, step_label in [
            ...
            if escaped:
                # Every later step would be planned against a nonsense target pose.
