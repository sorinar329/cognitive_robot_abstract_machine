...
Samples each cube's first attempt directly from the trained JPT's own belief of what a
successful attempt's parameters look like (see :func:`sample_actions` and
:meth:`~causal_diagnosis_v3.ActionCausalDiagnoser.sample_cause_values`), rather than
from a hand-specified Gaussian guess at the same thing -- ``demo3.py``'s and
``inference3a.py``'s own narrower range, or ``inference3b.py``'s own wider, deliberately
more failure-prone one. Otherwise structured exactly like both of those: the same
diagnose-and-correct loop, against the same trees, kicks in on any failure regardless of
where the failed attempt's parameters came from.

Run it with the interpreter whose packages point at this checkout, for example::

    /home/sorin/.virtualenvs/cram2-env/bin/python inference3c.py
"""
...
def apply_correction(
    ...
    :data:`~inference3c.MAX_CORRECTION_ATTEMPTS_PER_CUBE` one parameter at a time instead
