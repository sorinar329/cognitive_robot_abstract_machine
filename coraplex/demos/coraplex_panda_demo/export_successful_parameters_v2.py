...
rather than parameters the tree should learn a distribution over. ``iteration_index``
stays out on purpose even though it is an integer -- splitting on it would let a tree
separate runs by when they happened rather than by how they were parameterized. Every
cube's own final z position used to be bookkeeping here too; it now lives in
:data:`CUBE_FINAL_Z_COLUMNS` instead, since diagnosis needs the whole stack's state, not
just the acted-upon cube's, to catch a step that disturbed a cube it was not directly
placing.
"""
...
def read_successful_attempts(
    database_uri: str = SUCCESSFUL_DATABASE_URI,
) -> pandas.DataFrame:
    """
...
def parameters_for_learning(
    ...
    :param include_step: Whether to keep :data:`STEP_COLUMN`. Leave it out when every
        row is the same step already, where a constant column carries nothing.
    :raises MissingParameterColumns: If a requested parameter is absent from
        ``attempts``.
    """
...
def split_by_step(attempts: pandas.DataFrame) -> Dict[str, pandas.DataFrame]:
    ...
    :param attempts: Attempts as read from the database.
    """
    return {str(step_name): group for step_name, group in attempts.groupby("step_name")}
...
def export(output_directory: Path, per_step: bool, database_uri: str) -> None:
    ...
    Writes one file per action, which is what a model sampled from in the demo is
    learned from, and one holding every parameter for studying how the two actions
    relate.
