...
def persist_plan(
    ...
    :class:`~stacking_attempt_record_v2.StackingAttemptRecordV2` naming the iteration and
    step it came from and every cube's final z position.
    ...
    :param picked_cube_name: Name of the cube this step picked and placed, whose height
        in ``cube_final_heights`` becomes :attr:`StackingAttemptRecordV2.object_final_z`.
    """
    try:
...
with ExecutionEnvironment(
    ...
    for iteration in range(1, NUMBER_OF_ITERATIONS + 1):
        ...
        for cube_to_pick, cube_to_stack_on, step_label in [
            ...
            heights = cube_final_heights()
            iteration_plans.append(
                (step_label, plan, cube_to_pick.name.name, heights)
            )
            print(
                f"[info] {step_label} "
        ...
        for step_label, plan, picked_cube_name, heights in iteration_plans:
            persist_plan(
                plan, iteration, step_label, simulation_diverged, heights, picked_cube_name
            )
        print(
