"""
Trains a joint probability tree over the sampled physics parameters of successful pick
and place attempts (see ``export_successful_parameters.py``), and writes each fitted
model to JSON.
...
``export_successful_parameters.py``'s ``ACTION_PARAMETER_COLUMNS``). Each tree is
learned over its action's parameters together with ``step_index``, so it can tell which
of the three stacking steps a row describes rather than blending their distributions.
...
Run it with the interpreter whose packages point at this checkout, for example::
    ...
    /home/sorin/.virtualenvs/cram2-env/bin/python train_jpt.py
"""
...
DATASET_DIRECTORY = Path(__file__).parent.parent / "datasets"
"""
Where ``export_successful_parameters.py`` wrote the learning CSVs.
"""
...
"""
Minimum fraction of an action's attempts a leaf must cover.

Chosen coarse on purpose: the tree is a prior to sample new parameter combinations from,
not a lookup table of the training attempts, so leaves wide enough to average over
dozens of attempts generalize better than ones fit to a handful of points.
"""
...
def fit_tree(
    ...
    :param data: One action's sampled parameters, plus ``step_index``.
    """
