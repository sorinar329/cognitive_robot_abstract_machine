...
Reads ``models_v2/<name>_circuit.json`` -- the bare :class:`ProbabilisticCircuit`
...
Run it with the interpreter whose packages point at this checkout, for example::
    ...
    /home/sorin/.virtualenvs/cram2-env/bin/python export_posterior_plots.py
"""
...
MODELS_DIRECTORY = Path(__file__).parent / "models_v2"
"""
Where :data:`MODEL_NAMES`' ``<name>_circuit.json`` files are read from.
"""
...
def export_posterior(model_name: str) -> Path:
    ...
    """
    circuit_path = MODELS_DIRECTORY / f"{model_name}_circuit.json"
