"""
Second-generation copy of ``train_jpt.py``, training on ``export_successful_parameters_v2.py``'s
CSVs -- which add ``object_final_z`` to every action's learned columns (see that module's
own docstring) -- and writing the fitted trees under :data:`MODEL_DIRECTORY` so they do
not overwrite ``train_jpt.py``'s own models.
...
``export_successful_parameters_v2.py``'s ``ACTION_PARAMETER_COLUMNS``). Each tree is
...
and support the causal diagnosis in ``causal_diagnosis_v2.py``, which uses
...
Run it with the interpreter whose packages point at this checkout, for example::
    ...
    /home/sorin/.virtualenvs/cram2-env/bin/python train_jpt2.py
"""
...
DATASET_DIRECTORY = Path(__file__).parent.parent / "datasets_v2"
"""
Where ``export_successful_parameters_v2.py`` wrote the learning CSVs.
"""

MODEL_DIRECTORY = Path(__file__).parent / "models_v2"
"""
...
"""
Minimum fraction of an action's attempts a leaf must cover before the tree is allowed to
add another top-level decision split.
...
..note:: On this data, this bound is never actually the binding one -- the sampled
    parameters are close to independent of each other, so :class:`JointProbabilityTree`
    stops adding decision splits (finds no more impurity gain) long before hitting this
    floor regardless of its value; a sweep from 0.01 to 0.49 produced an identical tree.
    :data:`MIN_SAMPLES_PER_QUANTILE` is what actually controls how many pieces each
    leaf's own per-variable distribution is fit with.
"""

MIN_SAMPLES_PER_QUANTILE = 100
"""
Minimum number of an action's attempts each piece of a leaf's per-variable continuous
distribution must cover (passed to :func:`infer_variables_from_dataframe`, which
forwards it to each variable's
:class:`~probabilistic_model.learning.nyga_induction.NygaInduction` fit).

This, not :data:`MIN_SAMPLES_PER_LEAF`, is what actually governs how finely this data is
fit -- see that constant's own note. Raised from the library default (10) for the same
reason :data:`MIN_SAMPLES_PER_LEAF` is coarse: at the default, each action's tree came out
with 700-900+ leaves; a held-out log-likelihood sweep from 10 to 150 found the loss
monotonic and modest (pickup: 15.403 at 10 down to 14.601 at 100; place: 15.884 down to
15.365), while leaf count dropped roughly 90% (pickup: 871 to 90; place: 698 to 72) --
a clearly favorable trade for a sampling prior.
"""
...
Left at the library default: sweeping it from 0.01 to 0.3 changed nothing on this data
once :data:`MIN_SAMPLES_PER_QUANTILE` was the binding constraint.
"""

HOLDOUT_FRACTION = 0.1
"""
Fraction of an action's attempts withheld from fitting, to check the fitted tree assigns
a reasonable likelihood to attempts it never saw.
"""
...
class NonPositiveTrainingLikelihood(RuntimeError):
    ...
    A tree should always cover the data it was fit on; failing this means the fit itself
