...
class GiskardExecutable(Executable):
    ...
    kinematically teleported: each tick still forward-integrates a full
    ``control_dt`` of motion regardless of how much wall-clock time actually
    elapsed, so an unpaced loop makes Giskard's belief of a physically simulated
