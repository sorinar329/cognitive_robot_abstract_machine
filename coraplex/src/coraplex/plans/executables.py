...
class GiskardExecutable(Executable):
    ...
    """
    Whether :meth:`_execute_simulation`'s tick loop is paced to wall-clock time (via
    ...
    kinematically teleported: each tick still forward-integrates a full ``control_dt``
    of motion regardless of how much wall-clock time actually elapsed, so an unpaced
    loop makes Giskard's belief of a physically simulated DOF's position race ahead of
    where it has actually, physically settled.
    """
    ...
    The overall loop bound is this value multiplied by the number of motion mappings, so
    a stuck motion is bounded to roughly ``max_ticks_per_motion_mapping`` ticks before
    :class:`MotionDidNotFinish` is raised, instead of hanging (or taking minutes)
    indefinitely.
    """
        ...
        executor = Ros2Executor(
            context=MotionStatechartContext(
                ...
                qp_controller_config=QPControllerConfig(
                    target_frequency=50, prediction_horizon=20, verbose=False
                ),
            ),
