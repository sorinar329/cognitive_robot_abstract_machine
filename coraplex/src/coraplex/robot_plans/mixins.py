...
class HasMaxJointVelocity:
    ...
    """
    Maximum joint velocity (in rad/s or m/s, per joint), passed as the goal's own
    reference velocity rather than a hard limit. ``None`` leaves the speed
    unconstrained.
    """
...
class HasApproachVelocity:
    ...
    """
    Maximum linear speed (in m/s) for the initial pre-pose approach, passed as the
    goal's own reference velocity rather than a hard limit. ``None`` leaves the
    speed unconstrained.
    """
...
class ReachTuningParameters(HasApproachVelocity):
    ...
    """
    Maximum linear speed (in m/s) for the final approach onto the target pose, passed
    as the goal's own reference velocity rather than a hard limit. ``None`` leaves
    the speed unconstrained.
    """
...
class PickUpTuningParameters(ReachTuningParameters):
    ...
    """
    Maximum finger joint velocity (in m/s) used while closing onto the object, passed
    as the goal's own reference velocity rather than a hard limit. ``None`` leaves
    the speed unconstrained.
    """
    ...
    grasping, passed as the goal's own reference velocity rather than a hard
    limit. ``None`` leaves the speed unconstrained.
    """
...
class PlaceTuningParameters:
    ...
    passed as the goal's own reference velocity rather than a hard limit. ``None``
    ...
    location, before the final descent, passed as the goal's own reference
    velocity rather than a hard limit. ``None`` leaves the speed unconstrained.
    """
    ...
    the object, passed as the goal's own reference velocity rather than a hard
    limit. ``None`` leaves the speed unconstrained.
    """
    ...
    object, passed as the goal's own reference velocity rather than a hard limit.
...
class GripperStallToleranceParameters:
    ...
    """
    Maximum finger joint velocity (in m/s), passed as the goal's own reference
    velocity rather than a hard limit. ``None`` leaves the speed unconstrained.
    """
...
class CartesianVelocityLimitParameters:
    ...
    """
    Maximum linear speed (in m/s) of the tool center point, passed as the goal's own
    reference velocity rather than a hard limit. ``None`` leaves the linear speed
    unconstrained (other than the robot's own hardware limits).
    """

    max_angular_velocity: Optional[float] = field(default=None, kw_only=True)
    """
    Maximum angular speed (in rad/s) of the tool center point, passed as the goal's
    own reference velocity rather than a hard limit. Only meaningful for
    :class:`~giskardpy.motion_statechart.tasks.cartesian_tasks.CartesianPose` (i.e.
    when not :attr:`~coraplex.datastructures.enums.MovementType.TRANSLATION`).
