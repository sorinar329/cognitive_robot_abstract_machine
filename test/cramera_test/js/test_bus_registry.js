...
function load(file) {
  new Function(BrowserSource.read(path.join(WEB, file)))();
}
...
test('missing WebGL explains how to open the scene while inspection panels still mount', function () {
  const slots = freshDom();
  load('core/bus.js');
  load('core/registry.js');
  window.Panels.define('robot-scene', function () {
    throw new Error('Error creating WebGL context.');
  });
  window.Panels.define('eql', function () {});
  window.Panels.define('graph', function () {});
  window.CRAMERA_CONFIG = { layout: { left: ['robot-scene'], right: ['eql', 'graph'] } };
  const err = console.error; console.error = function () {};
  try {
    window.Panels.boot();
  } finally {
  ...
  assert.match(slots.left.children[0].innerHTML, /WebGL is unavailable/);
  assert.match(slots.left.children[0].innerHTML, /Open this address in a browser with WebGL enabled/);
