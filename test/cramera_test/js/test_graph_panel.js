...
const SOURCE = BrowserSource.read(path.join(WEB, 'panels/graph/panel.js'));

function loadResponseUtil() {
  const scope = {};
  new Function('window', BrowserSource.read(path.join(WEB, 'core/response.js')))(scope);
...
function loadSceneContext(search) {
  ...
  new Function('window', BrowserSource.read(path.join(WEB, 'core/scene.js')))(scope);
