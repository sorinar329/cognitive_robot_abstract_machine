...
def test_a_feed_listening_to_a_monitor_holds_what_the_monitor_detected(
    ...
    monitor = SegmindMonitor(world=world, detectors=detectors, listeners=[feed])

    for _ in range(TICKS_TO_WATCH):
        monitor.tick()

    assert feed.snapshot() == monitor.events
