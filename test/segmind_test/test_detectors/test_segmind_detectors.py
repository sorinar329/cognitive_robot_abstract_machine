...
def test_a_sudden_change_of_place_is_a_translation_from_where_the_body_rested(
    ...
    assert np.allclose(event.start_pose.position, rested_at.position)
    assert np.allclose(event.current_pose.position, milk.numeric_global_pose.position)
...
def test_a_change_of_place_after_a_translation_ended_runs_from_where_it_ended(
    ...
    assert np.allclose(later.start_pose.position, stopped.current_pose.position)
    _restore_milk(segmind_executor, milk)
