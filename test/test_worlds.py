...
from semantic_digital_twin.semantic_annotations.semantic_annotations import Cabinet, Cup, Table
...
def setup_support_world():
    ...
    table2_body = Body(
        name=PrefixedName("table2_body"), collision=shape_geometry, visual=shape_geometry
    )

    cylinder = Cylinder(width=0.1, height=0.2)
    shape_geometry = ShapeCollection([cylinder])
    cylinder_body = Body(
        name=PrefixedName("cylinder_body"), collision=shape_geometry, visual=shape_geometry
    )
    ...
    with world.modify_world():
        ...
        cabinet = Cabinet.create_with_new_body_in_world(world=world, scale=Scale(0.5, 0.5, 1.0), name=PrefixedName("cabinet"),
