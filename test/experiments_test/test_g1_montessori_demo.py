...
from experiments.montessori.g1_montessori_demo import (
...
EXPECTED_BASE_DOF_NAMES = {
    ...
    # Not base/leg joints at all: UnitreeG1LeftArm/UnitreeG1RightArm's own root is set
    # to "{side}_shoulder_pitch_link" (the shoulder pitch joint's *child* body), which
    # excludes that joint itself from the arm's kinematic chain (world.compute_chain_of_
    # connections(root, tip) never includes root's own parent connection) and so from
    # its degrees_of_freedom_with_hardware_interface too -- a pre-existing quirk of
    # semantic_digital_twin.robots.unitree_g1's own class definition, not something this
    # module or its tests introduce. Listed here because
    # _base_degrees_of_freedom_without_hardware_interface's contract is genuinely "every
    # active DOF not in the hardware-interface set", not "every leg DOF", and this is
    # what that set actually contains for the G1 today.
...
def montessori_with_robot():
    if not robot_installed(UnitreeG1):
        pytest.skip("iai_offis_g1_description is not installed")

    montessori = MontessoriWorld()
    montessori.spawn_robot(UnitreeG1)
    return montessori


def test_base_degrees_of_freedom_without_hardware_interface_finds_the_leg_joints(
